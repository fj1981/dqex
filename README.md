...
| AI 辅助写 SQL | ✅ | sql 内 `\ai` | — | 可选模块：智能体自动查询真实库表结构后生成 SQL（按目标方言），复用安全链路，未配置无入口 |
...
**AI 辅助写 SQL（可选模块，CLI `\ai` 与 Web AI 面板共用同一智能体）**：

- **智能体形态**：React Agent 工具调用模式——生成前自动调用 `list_databases` / `list_tables` / `get_schema` 查询真实库表结构（Web 端实时显示工具调用进度），不预注入字段，杜绝凭想象生成
- **生成 ≠ 执行**：AI 只产出 SQL 文本，复用 `dbx sql` 安全链路（写操作确认、危险函数拦截、审计）才落地；生成结果可 `\e` 编辑后执行
- **补全与应用**：`\ai continue` 基于上文续写；Web 端支持生成 / 解释 / 优化 / 修复，结果 diff 预览后一键应用到编辑器（替换选中 / 插入光标 / 追加）
- **密钥不出本机**：API Key 存本地，状态接口仅返回掩码端点与模型名，不回显明文
- **未配置无入口**：BaseURL/Model/Key 等四项非空才启用，不影响任何既有功能
