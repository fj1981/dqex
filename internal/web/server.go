...
func RunWeb(svc *service.Service, host string, port int, allow []string, noAuth, noBrowser bool) {
	...
	apiGroup := eb.Build(
		...
		eb.GROUP("/connections", []cygin.APIHandler{
			...
			eb.GET("/:id/databases", handleGetDatabases(svc)),
			eb.GET("/:id/db-schemas", handleGetDbSchemas(svc)),
			eb.GET("/:id/schema-objects", handleGetSchemaObjects(svc)),
			eb.GET("/:id/db-objects", handleGetDbObjects(svc)),
