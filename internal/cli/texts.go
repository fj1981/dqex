...
var cliTextsMap = map[string]cliTexts{
	"zh": {
		...
		urlTokenExpired: "⚠️  令牌已过期（签发给 24 小时有效期），请重启 Web 服务刷新后重新执行 dbx url",
		...
		startImport:  "开始导入...",
		doneImport:   "导入完成",
		warnReset:    "警告: 重置数据且未开启备份，目标表现有数据将无法恢复！",
		startMigrate: "开始迁移...",
		doneMigrate:  "迁移完成",
		startCompare: "开始对比...",
		...
		cmpNoPK:           "  （无主键，整行对比）",
		cmpDataSame:       "数据一致",
		cmpRowsDiff:       "行数不一致（超过阈值，仅比行数；可调大 --threshold 后重跑逐行对比）",
	...
	"en": {
		...
		urlTokenExpired: "⚠️  token expired (24h validity since issuance), restart the Web service and re-run dbx url",
		...
		warnReset:    "warning: resetting data without backup; existing data in target tables will be lost!",
		...
		cmpRowsDiff:       "row count differs (beyond threshold; only counts compared; raise --threshold and re-run for row-by-row)",
