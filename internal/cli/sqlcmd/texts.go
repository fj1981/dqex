...
type cliTexts struct {
	...
	errToolListDBs     string // 初始化 AI 查询工具失败
	errToolListTables  string // 初始化 AI 查询工具失败
	errToolGetSchema   string // 初始化 AI 查询工具失败
...
var cliTextsMap = map[string]cliTexts{
	"zh": {
		...
		needTTY:             "交互模式需要终端环境，请使用 -e 或 --json",
		metaHelp: `元命令:
  \\q, \\quit           退出
...
生成时自动查询真实表结构，无需手动刷新`,
		...
		aiEndpoint:             "  端点:   %s",
		aiModel:                "  模型:   %s",
		aiTuning:               "  温度: %.2f   回复上限: %d token   超时: %ds",
		aiSchemaLimit:          "  表结构文本上限: %d 字符",
		aiDebugLog:             "  debug 日志: %s（--debug 或配置文件 debug 开关）",
		aiDebugOn:              "开启",
		aiDebugOff:             "关闭",
		aiContext:              "  上下文: %d 条消息",
		aiProcTokens:           "启动以来累计 token:",
		aiProcTokensNone:       "启动以来累计 token: 尚无消耗",
		...
		toolRunning:            "  ⟳ %s (%s)...",
		errInitSvc:             "初始化服务失败",
		errAINotConfigured:     "AI 功能未配置：请在 config.yaml 或 Web 设置中填写 BaseURL / API Key / Model",
		errSchemaFail:          "获取表结构失败",
		errNoTargetDB:          "未找到可用数据库",
		errNoTableInTree:       "目标库 %s 不存在或不可访问",
		errToolListDBs:         "初始化 AI 查询工具失败",
		errToolListTables:      "初始化 AI 查询工具失败",
		errToolGetSchema:       "初始化 AI 查询工具失败",
		errReadFile:            "读取文件失败",
	...
	"en": {
		...
		needTTY:             "interactive mode requires a terminal, use -e or --json",
...
Real table structures are queried automatically during generation; no manual refresh needed`,
		...
		aiSchemaLimit:          "  schema text limit: %d chars",
		aiDebugLog:             "  debug log: %s (--debug or config debug switch)",
		aiDebugOn:              "on",
		aiDebugOff:             "off",
		aiContext:              "  context: %d messages",
		aiProcTokens:           "tokens since startup:",
		aiProcTokensNone:       "tokens since startup: none yet",
		...
		errAINotConfigured:     "AI is not configured: fill in BaseURL / API Key / Model in config.yaml or Web settings",
		errSchemaFail:          "failed to fetch table structure",
		errNoTargetDB:          "no available database found",
		errNoTableInTree:       "target database %s does not exist or is not accessible",
		errToolListDBs:         "failed to initialize AI query tools",
		errToolListTables:      "failed to initialize AI query tools",
		errToolGetSchema:       "failed to initialize AI query tools",
