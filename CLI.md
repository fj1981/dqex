...
| `--port` | `8181` | 监听端口 |
| `--allow` | 无 | 访问来源白名单，IP/CIDR/域名逗号分隔；本机回环始终放行；可选增强，进一步限制来源 IP |
| `--no-browser` | `false` | 启动时不自动打开浏览器 |
| `--no-auth` | `false` | 完全禁用令牌认证 |
| `--data-dir` | `~/.dqex` | 数据根目录 |
| `--config-file` | `~/.dqex/config.yaml` | 全局配置文件路径 |

> **安全说明**：令牌有效期 24 小时，未过期重启复用，超期后 API 返回 401 提示重启；`dqex url` 可随时取回访问链接。本机回环访问免认证；对外暴露（`--host 0.0.0.0`）时外部来源强制令牌认证，可选配置来源白名单进一步限制，同时支持暴力破解防护（1 分钟 10 次后锁定 5 分钟）。
>
